...
const KasOverview = () => {
  ...
  const groupByMainKategori = (data) => {
    ...
    Object.entries(data || {}).forEach(([kategori, total]) => {
      ...
      result[main].push({ sub: sub || 'Umum', total });
    });
    return result;
  };
  ...
  const pemasukan = groupByMainKategori(summary.pemasukanKategori);
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 grid-cols-1 lg:grid-cols-2">
        ...
        <div className="rounded-lg border bg-white p-4 shadow-sm">
          ...
          {Object.entries(pemasukan).map(([main, items]) => (
            <div key={main} className="mb-3">
              <div className="flex justify-between items-center">
                <span className="text-sm sm:text-base font-medium capitalize text-blue-700">{main}</span>
