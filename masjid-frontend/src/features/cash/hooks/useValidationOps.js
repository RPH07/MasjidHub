...
export const useValidationOps = (refreshCallback) => {
  ...
  const approveVoid = async (id) => {
    setLoading(true);
    try {
      const response = await cashService.approveVoid(id);
  ...
  const rejectVoid = async (id, rejectReason) => {
    setLoading(true);
    try {
      const response = await cashService.rejectVoid(id, rejectReason);
      if (refreshCallback) refreshCallback();
  ...
  const requestVoid = async (id, reason) => {
    setLoading(true);
    try {
      const response = await cashService.requestVoid(id, reason);
      if (refreshCallback) refreshCallback();
