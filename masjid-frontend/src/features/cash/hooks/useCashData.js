...
export const useCashData = (selectedPeriod) => {
  ...
  const fetchKasData = useCallback(async () => {
    ...
    try {
      const [summaryData, allData, masukData, keluarData, historyData] = await Promise.all([
        cashService.getSummary(filterParams),
        cashService.getTransactions({ ...filterParams, limit: 200 }),
        cashService.getTransactions({ ...filterParams, jenis: 'masuk', limit: 200 }),
        cashService.getTransactions({ ...filterParams, jenis: 'keluar', limit: 200 }),
        cashService.getHistory({ ...filterParams, type: 'all', status: 'all' })
      ]);
