...
const AdminTransparency = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const [zakat, programResponse] = await Promise.all([
        transparencyService.getZakat({ status: 'all' }),
        api.get('/pengadaan')
      ]);
      ...
      if (programId) {
        const report = await transparencyService.getProgram(programId, { status: 'all' });
  ...
  const handleCreateZakat = async (event) => {
    ...
    try {
      await transparencyService.createZakatDistribution(buildFormData(zakatForm));
  ...
  const handleCreateRealisasi = async (event) => {
    ...
    try {
      ...
      await transparencyService.createProgramRealisasi(programId, buildFormData(payload));
  ...
  const approveItem = async (type, item) => {
    ...
    try {
      if (type === 'zakat') {
        await transparencyService.approveZakatDistribution(item.id);
        await fetchData();
      } else {
        await transparencyService.approveProgramRealisasi(item.id);
  ...
  const rejectItem = async (type, item) => {
    ...
    try {
      if (type === 'zakat') {
        await transparencyService.rejectZakatDistribution(item.id, result.value);
        await fetchData();
      } else {
        await transparencyService.rejectProgramRealisasi(item.id, result.value);
        await handleProgramChange(selectedProgramId);
      }
