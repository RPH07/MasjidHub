...
fetchKasData = async () => {
  ...
  try {
    ...
    const [summaryResponse, kasManualResponse, zakatResponse, infaqResponse, lelangResponse] = await Promise.all([
      axios.get(`http://localhost:5000/api/kas/summary?period=${this.state.selectedPeriod}`, config),
      axios.get(`http://localhost:5000/api/kas/manual?period=${this.state.selectedPeriod}`, config),
    ...
    this.setState({
      ...
      pengeluaranKategori: {
        ...
        pemasukanKategoriManual: this.extractManualKategori(currentSummary.pemasukanKategori)
      },
      loading: false
    });
  } catch (error) {
...
extractManualKategori = (pemasukanKategori) => {
  const manualKategori = {};
  Object.keys(this.kategoriPemasukan).forEach(key => {
  ...
  closeBuktiModal = () => {
    this.setState({
      showBuktiModal: false,
      selectedBukti: null // Perbaikan: selectedBukti bukan selcetBukti
    });
  }
        ...
        <div className="border-b border-gray-200 -mx-4 px-4 sm:mx-0 sm:px-0">
          <nav className="flex space-x-2 sm:space-x-8 overflow-x-auto no-scrollbar">
            {[
              ...
              <button
                key={tab.key}
                className={`py-2 px-3 sm:px-1 border-b-2 font-medium text-sm whitespace-nowrap ${this.state.activeTab === tab.key
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-medium text-green-600 text-sm sm:text-base">
                    {this.formatCurrency(summary.pemasukanKategori.donasi || 0)}
                  </span>
                </div>
                  ...
                  if (amount > 0) {
                    return (
                      <div key={key} className="flex justify-between items-center pl-4 border-l-2 border-gray-200">
                        <span className="text-xs sm:text-sm text-gray-600">• {this.kategoriPemasukan[key]}</span>
        ...
        {this.state.activeTab === 'pemasukan' && (
          <div className="space-y-4">
                ...
                <table className="w-full min-w-[700px]">
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Nama/Deskripsi</th>
                  ...
                  <tbody className="divide-y divide-gray-200">
                    ...
                    {zakatData.map((item) => (
                      <tr key={`zakat-${item.id}`}>
                        ...
                        <td className="px-6 py-4 text-sm text-gray-900">
                          <span className={`px-2 py-1 text-xs font-medium rounded-full ${item.metode_pembayaran === 'qris'
                        ...
                        <td className="px-6 py-4">
                          <span className="text-gray-500 text-sm">Auto System</span>
                        </td>
                      </tr>
                    ))}
                    ...
                    {kasData.filter(item => item.jenis === 'masuk').map((item) => (
                      <tr key={`kas-${item.id}`}>
                        ...
                        <td className="px-6 py-4 text-sm text-gray-900">
                          {this.kategoriPemasukan[item.kategori_pemasukan] || 'Donasi Manual'}
                        </td>
            ...
            <div className="block sm:hidden space-y-4">
              {/* Data Zakat */}
              {zakatData.map((item) => (
                <div key={`zakat-mobile-${item.id}`} className="bg-white border rounded-lg p-4 shadow-sm">
              ...
              {/* Data Infaq */}
              {infaqData.map((item) => (
                <div key={`infaq-mobile-${item.id}`} className="bg-white border rounded-lg p-4 shadow-sm">
                  <div className="flex justify-between items-start mb-2">
                    <div className="text-sm text-gray-500">
                      {new Date(item.tanggal).toLocaleDateString('id-ID')}
                    </div>
              ...
              {/* Data Kas Masuk (Manual) */}
              {kasData.filter(item => item.jenis === 'masuk').map((item) => (
                <div key={`kas-mobile-${item.id}`} className="bg-white border rounded-lg p-4 shadow-sm">
                  <div className="flex justify-between items-start mb-2">
                    <div className="text-sm text-gray-500">
                      {new Date(item.tanggal).toLocaleDateString('id-ID')}
                    </div>
                  ...
                  {formData.jenis === 'masuk' && (
                    <div>
                      ...
                      <select
                        ...
                        required
                      >
                        {Object.entries(this.kategoriPemasukan).map(([key, label]) => (
